...
def load_transform(image_path, angle=0., s=(0, 0), size=(20, 20), var=0, amount=0.05):
    # Load the image
    original = np.abs(255 - imread(image_path, flatten=True))
    # Rotate the image
    rotated = np.maximum(np.minimum(rotate(original, angle=angle, mode='nearest', reshape=False), 1.), 0.)
    # Shift the image
    shifted = shift(rotated, shift=s)
    # Resize the image
    resized = np.asarray(scipy.misc.imresize(rotated, size=size), dtype=theano.config.floatX) / 255.
    # Invert the image
    distorted = add_noise(resized, mean=0, var=var, amount=amount, mode='pepper')
    # inverted = 1. - resized
