...
h5f = h5py.File('./saved_models/mnist_multi_class_changed/test/test_Results'
                '.h5', 'r')
...
mis_zeros = np.array([zeros[i] for i in range(zeros.shape[0]) if zeros[i, 1] >= 400])
cor_zeros = np.array([zeros[i] for i in range(zeros.shape[0]) if zeros[i, 1] <= 399])
...
mis_ones = np.array([ones[i] for i in range(ones.shape[0]) if ones[i, 1] >= 400])
cor_ones = np.array([ones[i] for i in range(ones.shape[0]) if ones[i, 1] <= 399])
