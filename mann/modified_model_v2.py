...
def memory_augmented_neural_network(input_var, target_var, batch_size=16,
                                    nb_class=10, nb_vals=9, memory_shape=(128, 40),
    ...
    W_xh, b_h = weight_and_bias_init((input_size + nb_vals, 4 * controller_size), name='xh')
    ...
    W_o, b_o = weight_and_bias_init((controller_size + nb_reads * memory_shape[1], nb_vals), name='o')
    gamma = 0.95
    ...
    # Model
    sequence_length_var = target_var.shape[0] / batch_size
    output_shape_var = (batch_size * sequence_length_var, nb_vals)

    # Join the input with time-offset labels
    one_hot_target_var_flatten = T.as_tensor_variable(target_var)
    one_hot_target_var = one_hot_target_var_flatten.reshape((batch_size, sequence_length_var, nb_vals))
    offset_target_var = T.concatenate([T.zeros_like(one_hot_target_var[:, 0]).dimshuffle(0, 'x', 1),
    ...
    o1_, o2_, o3_ = slice_equally(output_var_preactivation.reshape(output_shape_var), 3, 3)
    o1 = lasagne.nonlinearities.softmax(o1_)
    o2 = lasagne.nonlinearities.softmax(o2_)
    o3 = lasagne.nonlinearities.softmax(o3_)
    output_var_flatten = T.concatenate([o1, o2, o3], axis=1)
